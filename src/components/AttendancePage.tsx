...
import { attendanceRate, childAttendance, lessonDatesForMonth, shortWeekdayLabels, toggleAttendancePresence } from '../lib/attendance';
...
export const AttendancePage=memo(function AttendancePage({groups,attendance,onToggle,onEditGroups}:{groups:Group[];attendance:AttendanceLedger;onToggle:(groupId:string,date:string,childId:string)=>void;onEditGroups:()=>void}) {
  const [openGroupId,setOpenGroupId]=useState<string|null>(null);
  const [monthKey,setMonthKey]=useState(monthKeyNow());
  const [draftAttendance,setDraftAttendance]=useState(attendance);
  const draftAttendanceRef=useRef(attendance);
  const reportAttendance=useDeferredValue(draftAttendance);
  useEffect(()=>{if(!openGroupId){draftAttendanceRef.current=attendance;setDraftAttendance(attendance)}},[attendance,openGroupId]);
  ...
  const rate=group?attendanceRate(reportAttendance,group.id,group.children,reportDates):0;
  const openGroup=(groupId:string)=>{draftAttendanceRef.current=attendance;setDraftAttendance(attendance);setOpenGroupId(groupId)};
  const toggleCell=(date:string,childId:string)=>{if(!group)return;const next=toggleAttendancePresence(draftAttendanceRef.current,group.id,date,childId);draftAttendanceRef.current=next;setDraftAttendance(next);onToggle(group.id,date,childId)};
  useEffect(()=>{if(!group)return;const close=(event:KeyboardEvent)=>{if(event.key==='Escape')setOpenGroupId(null)};window.addEventListener('keydown',close);return()=>window.removeEventListener('keydown',close)},[group]);
