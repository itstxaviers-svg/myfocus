...
import { CalendarCheck2, ChevronLeft, ChevronRight, ClipboardCheck, Settings2, UsersRound } from 'lucide-react';
...
export function AttendancePage({groups,attendance,onAttendance,onEditGroups}:{groups:Group[];attendance:AttendanceLedger;onAttendance:(attendance:AttendanceLedger)=>void;onEditGroups:()=>void}) {
  ...
  return <section className="attendance-page">
    <div className="attendance-toolbar">
      ...
      <label><span>Group</span><select value={groupId} onChange={event=>setGroupId(event.target.value)}>{groups.map(item=><option value={item.id} key={item.id}>{item.name}</option>)}</select></label>
    ...
    {group&&group.scheduleWeekdays.length===0?<div className="attendance-empty panel"><Settings2 size={38}/><h2>Add class days</h2><p>Edit {group.name} and choose the weekdays when this group meets.</p><button className="primary" onClick={onEditGroups}>Edit group schedule</button></div>:group&&group.children.length===0?<div className="attendance-empty panel"><UsersRound size={42}/><h2>No children in this group</h2><p>Add children to {group.name} before marking attendance.</p><button className="primary" onClick={onEditGroups}>Manage groups</button></div>:group&&<>
      ...
      <div className="attendance-register panel">
        <div className="register-heading"><div><small>CLASS REGISTER</small><h2>{monthTitle(monthKey)}</h2></div><span>Tap a box to mark a child present</span></div>
        <div className="attendance-table-wrap"><table className="attendance-table"><thead><tr><th>Child</th>{dates.map(date=><th key={date} className={date>today?'future':''}><span>{dateLabel(date).split(' ')[0]}</span><strong>{Number(date.slice(-2))}</strong></th>)}</tr></thead><tbody>{group.children.map(child=><tr key={child.id}><th><span className="attendance-avatar">{child.name.slice(0,1).toUpperCase()}</span><span>{child.name}<small>{child.guardian||'Student'}</small></span></th>{dates.map(date=>{const checked=attendance[group.id]?.[date]?.includes(child.id)||false;return <td key={date} className={date>today?'future':''}><label className="attendance-check"><input type="checkbox" checked={checked} disabled={date>today} onChange={()=>toggle(date,child.id)} aria-label={`${child.name} attended ${dateLabel(date,true)}`}/><span>✓</span></label></td>})}</tr>)}</tbody></table></div>
      </div>

      {reportDates.length>0&&<div className="attendance-report panel">
        <div className="report-head"><div><small>{isFinal?'FINAL MONTHLY REPORT':'LIVE MONTHLY REPORT'}</small><h2>{group.name} · {monthTitle(monthKey)}</h2></div><div className="report-rate"><span>Group rate</span><strong>{rate}%</strong></div></div>
        <div className="report-list">{group.children.map(child=>{const result=childAttendance(attendance,group.id,child,reportDates);return <article key={child.id}><span className="attendance-avatar">{child.name.slice(0,1).toUpperCase()}</span><div><strong>{child.name}</strong><span>{result.attended.length} of {reportDates.length} classes attended</span></div><b>{result.rate}%</b><p>{result.missed.length?<><strong>Missed:</strong> {result.missed.map(date=>dateLabel(date,true)).join(', ')}</>:'No absences recorded.'}</p></article>})}</div>
      </div>}
    </>}
  </section>;
}
